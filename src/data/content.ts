...
export type PreviewDownloadTest = {
  enabled: boolean;
  fileLabel: string;
  notice: string;
  sources: { id: string; label: string; url: string; verification: "verified" | "manual_check" | "access_check"; note: string }[];
};
export type ProjectV2 = {
  schemaVersion: 2;
  slug: string;
  route: { code: string; order: number };
  game: { title: string; arabicTitle: string; series: { key: string; label: string; slug: string }; platforms: string[]; artwork: { coverUrl: string }; summary: string; story: string; };
  project: {
...
export const previewDownloadTests = publicationMode === "preview" && (import.meta.env.DEV || import.meta.env.ALPHA_LINE_ENABLE_DOWNLOAD_TESTS === "true");
...
const normalizeV2Project = (project: ProjectV2): Project => {
  const image = project.game.artwork.coverUrl;
  return {
    slug: project.slug,
    ...
    previewTestDownloads: project.previewTestDownloads,
